...
class RunningMeanStd:
    """Tracks the running mean/variance of a stream of vectors (Welford's online algorithm)."""
    ...
    def update(self, x: np.ndarray) -> None:
        ...
        new_var = (m_a + m_b + np.square(delta) * self.count * batch_count / total_count) / total_count
...
class RNDModel(nn.Module):
    ...
    A frozen, randomly-initialized target network and a trainable predictor
    network both map (normalized) observations to a feature vector. The
    predictor's MSE against the target is large for novel states and shrinks
    as the predictor learns to match the target on visited states, so it can
    be used as an intrinsic exploration reward.
    """

    def __init__(self, n_features: int, hidden_dim: int = 128, output_dim: int = 128):
        super().__init__()
