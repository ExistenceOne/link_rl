# Evolutionary Reinforcement Learning (ERL, Khadka & Tumer 2018) on top of TD3.
#
# A GA-evolved population of deterministic Actor networks plus one gradient-based TD3 learner share a
# single replay buffer. Population members are evaluated by REAL environment rollouts (fitness =
# episodic return); their transitions fill the buffer, TD3 (twin critic + target-policy smoothing +
# delayed policy updates) trains from the buffer, and the TD3 actor is periodically injected back
# into the population.
import os
import time
...
class ERL_TD3:
    def __init__(self, env: gym.Env, test_env: gym.Env, config: dict, use_wandb: bool):
        ...
        if use_wandb:
            self.wandb = wandb.init(project="erl_td3_{0}".format(self.env_name), name=self.current_time, config=config)
        else:
            self.wandb = None
        ...
        for generation in range(1, self.max_generations + 1):
            ...
            if self.time_steps > self.learning_starts and self.replay_buffer.size() > self.batch_size:
                ...
                for _ in range(num_updates):
                    actor_loss, critic_loss = self.td3_train_step()
    ...
    def model_save(self, validation_episode_reward_avg: float) -> None:
        filename = "erl_td3_{0}_{1:4.1f}_{2}.pth".format(self.env_name, validation_episode_reward_avg, self.current_time)
        torch.save(self.actor.state_dict(), os.path.join(MODEL_DIR, filename))
        copyfile(src=os.path.join(MODEL_DIR, filename), dst=os.path.join(MODEL_DIR, "erl_td3_{0}_latest.pth".format(self.env_name)))

    def model_save_periodic(self, generation: int) -> None:
        filename = "erl_td3_{0}_gen{1:06d}_{2}.pth".format(self.env_name, generation, self.current_time)
        torch.save(self.actor.state_dict(), os.path.join(MODEL_DIR, filename))
...
def main() -> None:
    ...
    use_wandb = True
    agent = ERL_TD3(env=env, test_env=test_env, config=config, use_wandb=use_wandb)
    agent.train_loop()
