...
class Actor(nn.Module):
    def __init__(self, n_features, n_actions):
        ...
        # ln(x) = -0.224 --> x = e^-0.224 = 0.799
        log_std_param = nn.Parameter(torch.full((n_actions,), -0.224))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.flatten(start_dim=-2)  # (B,4,24)->(B,96); (4,24)->(96,)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        mu_v = F.tanh(self.mu(x))

        std_v = self.log_std.exp()
        std_v = torch.clamp(std_v, min=0.15, max=0.8)  # Clamping for numerical stability
