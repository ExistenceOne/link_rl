# CEM-RL (Pourchot & Sigaud, 2019) on top of TD3.
#
# A diagonal Cross-Entropy-Method distribution N(mean, diag(var)) over the actor's flattened
# parameters samples a population of actors each generation. Half of them receive TD3 gradient
# steps (twin critic + target-policy smoothing + delayed policy updates) from a shared critic
# before being evaluated; ALL of them are evaluated by REAL environment rollouts (fitness =
# episodic return) and their transitions fill a shared replay buffer. The CEM distribution is then
# refit to the top-half elites.
import os
import time
...
from a_td3_models import MODEL_DIR, Actor, TwinQCritic, ReplayBuffer, DEVICE
...
class CEM_RL_TD3:
    def __init__(self, env: gym.Env, test_env: gym.Env, config: dict, use_wandb: bool):
        ...
        if use_wandb:
            self.wandb = wandb.init(project="cem_rl_td3_{0}".format(self.env_name), name=self.current_time, config=config)
        else:
            self.wandb = None
        ...
        self.twin_q_critic = TwinQCritic(self.n_features, self.n_actions, obs_ndim=self.obs_ndim)
        self.target_twin_q_critic = TwinQCritic(self.n_features, self.n_actions, obs_ndim=self.obs_ndim)
    ...
    def gradient_steps(self, theta_vec: torch.Tensor) -> torch.Tensor:
        ...
        for step in range(1, self.n_grad_steps + 1):
            ...
            critic_loss = F.mse_loss(q1_values, target_values) + F.mse_loss(q2_values, target_values)
            self.twin_q_critic_optimizer.zero_grad()
            critic_loss.backward()
            self.twin_q_critic_optimizer.step()

            if step % self.policy_update_delay == 0:
                actor_loss = -1.0 * self.twin_q_critic.q1_value(observations, self.actor(observations)).mean()
    ...
    def model_save(self, validation_episode_reward_avg: float) -> None:
        ...
        filename = "cem_rl_td3_{0}_{1:4.1f}_{2}.pth".format(self.env_name, validation_episode_reward_avg, self.current_time)
        torch.save(self.eval_actor.state_dict(), os.path.join(MODEL_DIR, filename))
        copyfile(src=os.path.join(MODEL_DIR, filename), dst=os.path.join(MODEL_DIR, "cem_rl_td3_{0}_latest.pth".format(self.env_name)))

    def model_save_periodic(self, generation: int) -> None:
        vector_to_parameters(self.mean, self.eval_actor.parameters())
        filename = "cem_rl_td3_{0}_gen{1:06d}_{2}.pth".format(self.env_name, generation, self.current_time)
        torch.save(self.eval_actor.state_dict(), os.path.join(MODEL_DIR, filename))
...
def main() -> None:
    ...
    use_wandb = True
    agent = CEM_RL_TD3(env=env, test_env=test_env, config=config, use_wandb=use_wandb)
    agent.train_loop()
