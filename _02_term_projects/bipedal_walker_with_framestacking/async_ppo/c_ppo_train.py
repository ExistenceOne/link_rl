...
class PPO:
    def __init__(self, config: dict, use_wandb: bool):
        ...
        # Determine observation/action dimensions from a frame-stacked env
        temp_env = gym.make(config["env_name"])
        temp_env = gym.wrappers.FrameStackObservation(temp_env, stack_size=4)
        n_features = int(np.prod(temp_env.observation_space.shape))  # (4,24) -> 96
        n_actions = temp_env.action_space.shape[0]
        temp_env.close()

        # Initialize global models and optimizers
        self.global_actor = Actor(n_features=n_features, n_actions=n_actions).share_memory()
        self.global_critic = Critic(n_features=n_features).share_memory()
...
def main() -> None:
    ...
    config = {
        ...
        "ppo_epochs": 5,                                   # PPO 내부 업데이트 횟수
        "ppo_clip_coefficient": 0.1,                        # PPO Ratio Clip Coefficient
