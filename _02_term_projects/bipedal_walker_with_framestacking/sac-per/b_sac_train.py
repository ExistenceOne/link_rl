...
class SAC:
    def __init__(self, env: gym.Env, test_env: gym.Env, config: dict, use_wandb: bool):
        ...
        if use_wandb:
            self.wandb = wandb.init(project="sac_per_{0}_with_framestacking".format(self.env_name), name=self.current_time, config=config)
        else:
            self.wandb = None
        ...
        self.per_beta_frames = config["per_beta_frames"]
    ...
    def load_checkpoint(self, checkpoint_path: str, load_alpha: bool = False, load_optimizers: bool = False) -> None:
        ...
        if "per_frame" in checkpoint:
            self.replay_buffer.frame = checkpoint["per_frame"]
        if "per_max_priority" in checkpoint:
            self.replay_buffer.max_priority = checkpoint["per_max_priority"]
        ...
        self.wandb.log(
            {
                ...
                "[TRAIN] PER beta": self.replay_buffer._beta(),
    ...
    def train(self):
        ...
        observations, actions, next_observations, rewards, dones, indices, weights = self.replay_buffer.sample(self.batch_size)
        ...
        qf1_loss = (weights * F.mse_loss(qf1, target_values, reduction="none")).mean()  # JQ = 𝔼(st,at)~D[0.5(Q1(st,at) - r(st,at) - γ(𝔼st+1~p[V(st+1)]))^2]
        qf2_loss = (weights * F.mse_loss(qf2, target_values, reduction="none")).mean()  # JQ = 𝔼(st,at)~D[0.5(Q1(st,at) - r(st,at) - γ(𝔼st+1~p[V(st+1)]))^2]
        ...
        # PER priority update based on the TD-error magnitude observed for the sampled transitions
        with torch.no_grad():
            td_errors = (torch.abs(qf1 - target_values) + torch.abs(qf2 - target_values)) / 2.0
        self.replay_buffer.update_priorities(indices, td_errors.squeeze(-1).cpu().numpy())
    ...
    def model_save(self, validation_episode_reward_avg: float) -> None:
        filename = "sac_per_{0}_{1:4.1f}_{2}.pth".format(self.env_name, validation_episode_reward_avg, self.current_time)
        torch.save(self.policy.state_dict(), os.path.join(MODEL_DIR, filename))

        copyfile(src=os.path.join(MODEL_DIR, filename), dst=os.path.join(MODEL_DIR, "sac_per_{0}_latest.pth".format(self.env_name)))
        ...
        checkpoint = {
            ...
            "per_max_priority": self.replay_buffer.max_priority,
        }
        if self.automatic_entropy_tuning:
            checkpoint["log_alpha"] = self.log_alpha.detach().cpu()
            checkpoint["alpha_optimizer"] = self.alpha_optimizer.state_dict()

        torch.save(checkpoint, os.path.join(MODEL_DIR, "sac_per_{0}_latest_checkpoint.pth".format(self.env_name)))
...
def main() -> None:
    ...
    config = {
        ...
        "per_alpha": 0.6,                # priority exponent (0 = uniform sampling, 1 = full prioritization)
